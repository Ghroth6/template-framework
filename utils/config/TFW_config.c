...
// 配置管理器结构
typedef struct {
    char configPath[TFW_PATH_LEN_MAX];
...
int32_t TFW_Config_Initialize(const char* configPath) {
    ...
    // 初始化配置数组
    g_configManager.configs = (TFW_ConfigItem*)TFW_Malloc(TFW_CONFIG_KEY_COUNT * sizeof(TFW_ConfigItem));
    ...
    for (int i = 0; i < TFW_CONFIG_KEY_COUNT; i++) {
        ...
        if (defaultValue) {
            // 使用统一内存管理函数
            size_t len = strlen(defaultValue) + 1;
            char* newValue = (char*)TFW_Malloc(len);
            if (newValue) {
                strncpy(newValue, defaultValue, len);
                g_configManager.configs[i].value.string_value = newValue;
            }
        }
    }
    ...
    // 初始化状态
    TFW_Memset_S(&g_configManager.status, sizeof(g_configManager.status), 0, sizeof(g_configManager.status));
    ...
    if (result != TFW_SUCCESS) {
        ...
        g_configManager.status.lastLoadTime = TFW_GetTimestampMs();
    } else {
        // 配置加载成功
        g_configManager.status.loaded = true;
        g_configManager.status.lastLoadTime = TFW_GetTimestampMs();
...
int32_t TFW_Config_SetString(TFW_ConfigKey key, const char* value) {
    ...
    if (config->type == TFW_CONFIG_TYPE_STRING) {
        ...
        if (config->value.string_value) {
            TFW_Free((void*)config->value.string_value);
        }
        ...
        config->value.string_value = newValue;

        g_configManager.status.lastSaveTime = TFW_GetTimestampMs();
...
int32_t TFW_Config_GetInt(TFW_ConfigKey key, int32_t* value, int32_t defaultValue) {
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_INT && config->value.int_value != 0) {
        *value = config->value.int_value;
        TFW_Mutex_Unlock(&g_configManager.configMutex);
        return TFW_SUCCESS;
    }
...
int32_t TFW_Config_GetBool(TFW_ConfigKey key, bool* value, bool defaultValue) {
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_BOOL && config->value.bool_value != false) {
        *value = config->value.bool_value;
        TFW_Mutex_Unlock(&g_configManager.configMutex);
        return TFW_SUCCESS;
    }
...
int32_t TFW_Config_GetFloat(TFW_ConfigKey key, float* value, float defaultValue) {
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_FLOAT && config->value.float_value != 0.0f) {
        *value = config->value.float_value;
        TFW_Mutex_Unlock(&g_configManager.configMutex);
        return TFW_SUCCESS;
    }
...
int32_t TFW_Config_SetInt(TFW_ConfigKey key, int32_t value) {
    if (!g_configManager.initialized || key >= TFW_CONFIG_KEY_COUNT) {
        return TFW_ERROR_INVALID_PARAM;
    }
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_INT) {
        config->value.int_value = value;

        g_configManager.status.lastSaveTime = TFW_GetTimestampMs();
        TFW_LOGI_CORE("Config updated: %s = %d", g_configKeyNames[key], value);
...
int32_t TFW_Config_SetBool(TFW_ConfigKey key, bool value) {
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_BOOL) {
        config->value.bool_value = value;

        g_configManager.status.lastSaveTime = TFW_GetTimestampMs();
        TFW_LOGI_CORE("Config updated: %s = %s", g_configKeyNames[key], value ? "true" : "false");
...
int32_t TFW_Config_SetFloat(TFW_ConfigKey key, float value) {
    ...
    TFW_ConfigItem* config = &g_configManager.configs[key];
    if (config->type == TFW_CONFIG_TYPE_FLOAT) {
        config->value.float_value = value;

        g_configManager.status.lastSaveTime = TFW_GetTimestampMs();
        TFW_LOGI_CORE("Config updated: %s = %f", g_configKeyNames[key], value);
...
int32_t TFW_Config_GetStatus(TFW_ConfigStatus* status) {
    ...
    TFW_Mutex_Lock(&g_configManager.configMutex);
    TFW_Memcpy_S(status, sizeof(TFW_ConfigStatus), &g_configManager.status, sizeof(TFW_ConfigStatus));
    TFW_Mutex_Unlock(&g_configManager.configMutex);
    ...
    if (g_configManager.configs) {
        for (int i = 0; i < TFW_CONFIG_KEY_COUNT; i++) {
            if (g_configManager.configs[i].type == TFW_CONFIG_TYPE_STRING &&
                ...
                TFW_Free((void*)g_configManager.configs[i].value.string_value);
            }
        }
        TFW_Free(g_configManager.configs);
